...
# 新增商店子分類
def create(request):
    template = get_template('shop_sub_category/create.html')
...
def save(request):
    ...
    if request.method == 'POST':
        ...
        c_shop_sub_category = request.POST.get('c_shop_sub_category', '')
        e_shop_sub_category = request.POST.get('e_shop_sub_category', '')
        unselected_shop_sub_category_icon = request.FILES.get('unselected_shop_sub_category_icon','')
        selected_shop_sub_category_icon = request.FILES.get('selected_shop_sub_category_icon','')
        # shop_category_background_color = request.POST.get('shop_category_background_color', '')
        shop_sub_category_seq = request.POST.get('shop_sub_category_seq', '')
        # 檢查欄位是否填寫
        if response_data['status'] == 0:
            if not(c_shop_sub_category):
                response_data['status'] = -1
                response_data['ret_val'] = '中文商店分類未填寫!'
                response_data['test'] =shop_category_id

        if response_data['status'] == 0:
            if not(e_shop_sub_category):
                response_data['status'] = -2
                response_data['ret_val'] = '英文商店分類未填寫!'

        if response_data['status'] == 0:
            if not(unselected_shop_sub_category_icon):
                response_data['status'] = -3
                response_data['ret_val'] = '未上傳 unselected_shop_category_icon!'
                response_data['test'] =unselected_shop_sub_category_icon

        if response_data['status'] == 0:
            if not(selected_shop_sub_category_icon):
        ...
        if response_data['status'] == 0:
            if not(shop_sub_category_seq):
                response_data['status'] = -7
                response_data['ret_val'] = '未填寫商店分類排序!'
        # 檢查欄位格式是否正確
        if response_data['status'] == 0:
            if not(re.match('^[\u4e00-\u9fa5]{1,50}$', c_shop_sub_category)):
                response_data['status'] = -8
                response_data['ret_val'] = '中文商店分類格式錯誤!'

        if response_data['status'] == 0:
            if not(re.match('^[A-Za-z]{1,50}$', e_shop_sub_category)):
                response_data['status'] = -9
                response_data['ret_val'] = '英文商店分類格式錯誤!'
#--------------------------------------------------------
#暫時註解掉
        # if response_data['status'] == 0:
        #     if not(re.match('^\w+\.(gif|png|jpg|jpeg)$', str(unselected_shop_sub_category_icon.name))):
        #         response_data['status'] = -10
        #         response_data['ret_val'] = 'unselected_shop_category_icon 格式錯誤!'

        # if response_data['status'] == 0:
        #     if not(re.match('^\w+\.(gif|png|jpg|jpeg)$', str(selected_shop_sub_category_icon.name))):
...
#--------------------------------------------------------
        ...
        if response_data['status'] == 0:
            if not(re.match('^\d+$', shop_sub_category_seq)):
        ...
        if response_data['status'] == 0:
            ...
            models.Shop_Sub_Category.objects.create(
                ...
                unselected_shop_sub_category_icon=unselected_shop_sub_category_icon, 
                selected_shop_sub_category_icon=selected_shop_sub_category_icon, 
                # shop_category_background_color=shop_category_background_color,  
                shop_sub_category_seq=shop_sub_category_seq
